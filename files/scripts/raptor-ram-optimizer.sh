...
def run_sudo(cmd):
    ...
    try:
        ...
        # fallback: try sudo (for terminal use)
        try:
            subprocess.run(
                ["sudo", "bash", "-c", cmd],
                check=False, capture_output=True
...
# Each entry: (display_name, pkill_pattern, resume_on_exit)
SUSPENDABLE = [
...
SUSPENDED = []   # populated at runtime
...
class RaptorRAMWindow(Adw.ApplicationWindow):
    ...
    def _refresh_stats(self):
        ...
        zram_orig, zram_mem = get_zram_usage()
        if zram_orig > 0 and zram_mem > 0:
            ratio = zram_orig / zram_mem if zram_mem > 0 else 0
            self.zram_label.set_text(
                f"{fmt_mb(zram_orig)} data → {fmt_mb(zram_mem)} RAM  "
    ...
    def _do_optimize(self, opts, before_used, before_total):
        ...
        # 6. OOM score adjustment — protect foreground apps
        if opts["oom"]:
            # Lower OOM score for the current desktop session processes
            run_sudo(
    ...
    def on_resume(self, btn):
        ...
        # If gaming mode was on, drop back to balanced
        subprocess.run(
