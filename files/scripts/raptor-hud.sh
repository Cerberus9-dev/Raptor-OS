...
# Copy GPU profile script to runtime location
mkdir -p /usr/local/bin
cp /tmp/raptor-gpu-profile.sh /usr/local/bin/raptor-gpu-profile.sh 2>/dev/null || true
chmod +x /usr/local/bin/raptor-gpu-profile.sh 2>/dev/null || true
