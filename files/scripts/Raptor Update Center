...
# Installs the Python GUI updater and its .desktop entry.
# ═══════════════════════════════════════════════════════════════════════════════
...
class RaptorUpdateWindow(Adw.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        icon = Gtk.Image.new_from_icon_name("system-software-update")
...
# ── Wrapper script so KDE launcher doesn't choke on inline env vars ────────────
# Exec= with "env VAR=val command" is unreliable in Plasma; a wrapper is safer.
cat << 'EOF' > /usr/bin/raptor-update-launcher
#!/bin/bash
# Wrapper for Raptor Update Center — sets Adwaita portal env before launch
export ADW_DISABLE_PORTAL=1
export GDK_BACKEND=x11          # fallback if Wayland compositor lacks GTK4 support
exec /usr/bin/raptor-update "$@"
EOF
chmod +x /usr/bin/raptor-update-launcher
 
# ── Icon — ship our own SVG so it works on any theme ──────────────────────────
# KDE will find icons in /usr/share/icons/hicolor before falling back to theme.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="64" height="64">
  ...
  <!-- Down arrow (update/download symbol) -->
  <polyline points="32,14 32,42" stroke="#33ff33" stroke-width="5" stroke-linecap="round"/>
...
# Also drop it at the common 48x48 and 256x256 symlink sizes KDE checks
...
# Rebuild the GTK icon cache so the new icon is found immediately
...
# Use the wrapper script — avoids inline-env parsing bugs in Plasma
Exec=/usr/bin/raptor-update-launcher
# Our own hicolor icon — works on every theme
Icon=raptor-update
Terminal=false
...
# Rebuild sysoca so KDE picks up the new .desktop immediately at image boot
