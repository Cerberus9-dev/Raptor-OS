...
Installation
Download the latest ISO from Internet Archive, then flash it to a USB drive using one of the following:

Rufus — when prompted, select DD image mode and GPT partition scheme
Ventoy
Fedora Media Writer — guaranteed to work if others fail

Updating

If you already have Raptor OS installed, open Raptor Update Center from the app menu — no terminal needed.
...
Raptor Update Center — update Raptor OS with one click, no terminal required
...
BlueBuild — custom image build system
Bazzite — base image
